...
**In development, tested but not released:** `/huimem doctor` reports stale or missing evidence, unavailable required records, ADRs without a basis section and task-file drift. It opens the existing database read-only and applies no repairs. See the [diagnostic guide](docs/GUIDE.md#doctor). Known gaps from the 16 September check are listed in [validation](docs/VALIDATION.md). This command is not part of the v0.6.2 release below.
