...
> **预览版 v0.6.2，已在 OMP 18.1.5 上测试。** 暂不支持原版 Pi。这个插件帮助恢复和检查项目上下文，但不能消除模型幻觉。
...
**开发中，已测试，尚未发布：** `/huimem doctor` 报告过期或丢失的证据、不可用的必需记录、缺少依据部分的 ADR，以及任务文件与记录库的差异。它以只读方式打开现有数据库，不执行修复。详见[诊断指南](../GUIDE.md#doctor)。9 月 16 日检查发现的不足列于[验证记录](../VALIDATION.md)。下方的 v0.6.2 发行版尚不包含此命令。
...
**设置界面：** 在终端中，`/huimem` 会打开一个由 OMP 原生对话框构成的界面。每一行显示当前值；
按 Enter 修改该值或执行操作，按 Esc 关闭。在这里可以暂停保存、设置召回预算与注入上限、从记录库中
选择必需记录、重新发布 `RECORDS.md`、查看最近的记忆块、迁移旧 ADR 以及重置上限。在未启用记忆的
项目中，同一界面会提供启用选项。取值按项目保存在 `.memory/settings.json`。在 RPC 和 `-p` 模式下，
`/huimem` 以文本输出状态，所有操作仍可通过子命令使用（`/huimem help`）。`/huimem omp` 列出影响记忆的
OMP 设置：插件只展示，不改写他人配置。
...
提示中列出，改为按 ID 读取，腾出的空间留给与请求匹配的记录（如果放得下）。不存在或已停用的 ID 也会在其中列出。该提示
...
记录版本或文件哈希。只有当这些依据发生变化、被停用或丢失时，该记录才会变为 STALE，并显示原因
（`db changed: version 1 -> 2`），沿记录链同样适用。无关记录不受影响。只有保存一个带当前依据的新版本
才能清除 STALE。没有 `dependsOn` 的记录（包括所有旧记录）沿用原规则：任何规范文档变化都会将其标记为 STALE。

**并行任务：** 当 commit 恰好修改一条任务记录或传入 `task=<任务 ID>` 时，检查点摘要归属该任务。
记忆块随后以 `Task checkpoints` 列出，每条摘要位于其任务 ID 之下，与请求匹配的任务排在最前，
因此一个任务的下一步不会被当作另一个任务的下一步。没有任务的摘要会带时间单独显示；
按任务 ID 调用 `project_memory recall` 会返回其最近的检查点。git 分支仅记录用于诊断。
单独的 `git worktree` 拥有自己的 `.memory/runtime`，看不到主检出目录中的记录。
...
安装 Bun 后，`bun run build` 构建扩展，`bun run check` 执行测试及包检查。同一组适配器测试同时验证源码和构建产物；最新测试数量见 VALIDATION.md。无需额外运行时 npm 依赖，构建产物已包含在仓库中。
