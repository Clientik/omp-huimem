...
[На главную](i18n/README.ru.md) · [Руководство](GUIDE.md)
...
В рабочей версии commit также автоматически создаёт `.memory/RECORDS.md`: читаемое представление последних версий SQLite. Это производный файл, не источник новых свидетельств. Желаемый текст сохраняется в базе вместе с записью, публикация восстанавливается при открытии или через `/huimem sync`. Ручные канонические документы по-прежнему поддерживает человек или основная модель. Подробности и миграция схемы — в [руководстве](GUIDE.md).
...
В рабочей версии после v0.4.1 новые файловые факты (`kind: fact` с `source.path`) проверяют свой источник и общий отпечаток MEMORY.md, PROJECT.md и architecture.json. Посторонний ADR или todo.json больше не делает их STALE. Если сам источник — ADR или todo.json, его изменение по-прежнему обнаруживается. Это проверка файлового наблюдения, не доказательство согласованности со всеми решениями проекта.

Решения из переписки, остальные виды записей и старые версии без нового отпечатка сохраняют общую консервативную проверку. Старые записи не переводятся автоматически: новый порядок применяется при их следующем проверенном commit. Схема SQLite остаётся 2; прежний плагин проигнорирует дополнительное поле и применит более строгую проверку. `STALE` не удаляет запись и не исправляет её автоматически. Зависимости через links и смена ветки отдельно не отслеживаются.
